...
run_overrepresentation_analysis <- function(gene_set, dds_res, 
  ...
  if(type == "reactome"){
      ...
      entrez_ids.df <- clusterProfiler::bitr(geneID = geneList.filtered, fromType = "ENSEMBL", toType = "ENTREZID", OrgDb="org.Sc.sgd.db")
  ...
  if(dim(msig_h.oa@result)[1] > 0){
      ...
      msig_h.oa.gs <- setReadable(msig_h.oa, org.Sc.sgd.db, keyType = "ENSEMBL")
...
run_enrichment_analysis <- function(gene_set, geneList, analysis_name = "dds.result", gs_name = "MSigDB", adj_p_cutoff = 0.05, type = "general"){
  ...
  if(type == "reactome"){
    # Replace ensembl IDs by Entrez IDs for compatibility with gsePathway
    entrez_ids.df <- clusterProfiler::bitr(geneID = names(geneList), fromType = "ENSEMBL", toType = "ENTREZID", OrgDb="org.Sc.sgd.db")
    ...
    if(dim(msig_h.gsea@result)[1] > 0){ 
      ...
      msig_h.gsea.gs <- setReadable(msig_h.gsea, org.Sc.sgd.db, keyType = "ENTREZID")
    } 
  } else {
    msig_h.gsea <- GSEA(geneList, TERM2GENE = gene_set, pvalueCutoff = adj_p_cutoff, verbose = TRUE, eps = 0)
    if(dim(msig_h.gsea@result)[1] > 0){ 
      # Convert Entrez gene IDs to gene symbols
      msig_h.gsea.gs <- setReadable(msig_h.gsea, org.Sc.sgd.db, keyType = "ENSEMBL")
    }  
...
go_overrep <- function(dds_res, my_file, golevel = 3, 
  ...
  # Save Log2FC vals in log2fc_symb.df using gene symbols as IDs. 
  log2fc_symb.df <- as.data.frame(dds_res[,c("log2FoldChange","padj")])
  log2fc_symb.df$symbols <- replace_gene_acc_by_gb_ids(rownames(log2fc_symb.df),
                                                             return_all = TRUE)
  ...
  keep <- grep('^[0-9]+$', rownames(dds_res), invert = FALSE)
  ...
  ego <- setReadable(ego , org.Sc.sgd.db, keyType = "ENTREZID", keyoutput = "GENENAME")
...
go_classification <- function(dds_res, my_file, golevel = 3, onthology = "BP", log2FC_cutoff = 0, alpha = 1){
  ...
  ggo <- groupGO(gene     = geneList.filtered,
                 ...
                 readable = FALSE, # if readable is TRUE, the gene IDs will mapping to gene symbols.
                 keyType = 'ENTREZID'
  )
...
go_gsea <- function(dds_res, my_file, golevel = 3, 
  ...
  go.gsea.gs <- setReadable(go.gsea , org.Sc.sgd.db, keyType = "ENTREZID", keyoutput = "GENENAME")
  
  print("Ran GSEA analysis")
  ...
  if(length(go.gsea.gs@result$NES) > 0){
    ...
    if(dim(go.gsea_bt_0)[1] > 1){ # We need at least 2 nodes for a network or tree plot
      ...
      # Network plot
      print("Plotting network plots 2")
      p8 <- emapplot(distance_matrix.up, 
                     repel = T, 
                     showCategory = 200, 
                     legend_n = 5, 
    ...
    if(dim(go.gsea_lt_0)[1] > 1){ # We need at least 2 nodes for a network or tree plot
      ...
      # Network plot
      print("Plotting network plots 3")
      p10 <- emapplot(distance_matrix.down, 
                      repel = T, 
                      showCategory = 200, 
                      legend_n = 5, 
  ...
  print(paste("Returning", onthology))
  return(go.gsea.gs)
}
